...
import { updateMemory } from '@/actions/memory'
...
import { CalendarIcon, Loader2, ImagePlus, MapPin, Heart, X } from 'lucide-react'
...
export function EditMemoryDialog({ open, onOpenChange, memory }: EditMemoryDialogProps) {
  ...
  useEffect(() => {
    if (memory) {
      form.reset({
        ...
        eventDate: new Date(memory.eventDate),
  ...
  const onSubmit = async (data: MemoryFormData) => {
    ...
    try {
      const result = await updateMemory(memory.id, data)
      if (result.success) {
        onOpenChange(false)
      } else {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl bg-stone-900 border-stone-800 text-stone-100 max-h-[90vh] overflow-y-auto scrollbar-thin scrollbar-thumb-stone-700">
        <DialogHeader>
          <DialogTitle className="text-2xl font-serif text-rose-200">Edit Memory</DialogTitle>
        </DialogHeader>
